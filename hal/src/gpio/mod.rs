    ...
    Self: HL,
{
    ...
    fn cr_modify(&mut self, _cr: &mut <Self as HL>::Cr, f: impl FnOnce(u32) -> u32) {
        ...
        match N {
            0..=7 => {
                gpio.cfglr.modify(|r, w| unsafe { w.bits(f(r.bits())) });
            }
            8..=15 => {
                gpio.cfghr.modify(|r, w| unsafe { w.bits(f(r.bits())) });
            }
            _ => unreachable!(),
        }
    }
    ...
    Self: HL,
{
    fn mode<MODE: PinMode>(&mut self, cr: &mut <Self as HL>::Cr) {
        ...
        if let Some(pull) = MODE::PULL {
            if pull {
                gpio.bshr.write(|w| unsafe { w.bits(1 << N) });
            } else {
                gpio.bshr.write(|w| unsafe { w.bits(1 << (16 + N)) });
            }
        }
