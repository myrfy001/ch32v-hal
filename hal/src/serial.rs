...
remap!(
    USART1, PA9, PA10 => { |_, w| w.usart1rm().bit(false) };
    USART1, PB6, PB7  => { |_, w| w.usart1rm().bit(true) };

    USART2, PA2, PA3 => { |_, w| w.usart2rm().bit(false) };
    USART2, PD5, PD6 => { |_, w| w.usart2rm().bit(true) };

    USART3, PB10, PB11 => { |_, w| unsafe { w.usart3rm().bits(0b00)} };
    USART3, PC10, PC11 => { |_, w| unsafe { w.usart3rm().bits(0b01)} };
    USART3, PD8, PD9 => { |_, w| unsafe { w.usart3rm().bits(0b11)} };
);
...
impl<USART: Instance, PINS> Serial<USART, PINS> {
    ...
    ///
    /// `PCFR` and `APBX` are register handles which are passed for
    /// configuration. (`PCFR` is used to map the USART to the
    ...
    ///
    /// ```
    /// let mut serial = Serial::new(usart, (tx_pin, rx_pin), &mut afio.pcfr, 9600.bps(), &clocks);
    ///
...
fn apply_config<USART: Instance>(config: Config, clocks: &Clocks) {
    ...
    usart.ctlr2.modify(|_r, w| unsafe{w.stop().bits(stop_bits)});
}
...
impl<USART: Instance> Tx<USART> {
    ...
    pub fn write_u16(&mut self, word: u16) -> nb::Result<(), Infallible> {
        ...
        if usart.statr.read().txe().bit_is_set() {
            usart.datar.write(|w| unsafe{w.dr().bits(word)});
            Ok(())
        } else {
    ...
    pub fn listen(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.txeie().set_bit()) };
    }

    /// Stop listening for transmit interrupt event
    pub fn unlisten(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.txeie().clear_bit()) };
    }

    /// Returns true if the tx register is empty (and can accept data)
    pub fn is_tx_empty(&self) -> bool {
        unsafe { (*USART::ptr()).statr.read().txe().bit_is_set() }
    }

    pub fn is_tx_complete(&self) -> bool {
        unsafe { (*USART::ptr()).statr.read().tc().bit_is_set() }
    }
}
...
impl<USART: Instance> Rx<USART> {
    ...
    pub fn read_u16(&mut self) -> nb::Result<u16, Error> {
        ...
        if let Some(err) = err {
            ...
            let _ = usart.statr.read();
            let _ = usart.datar.read();
            Err(nb::Error::Other(err))
        } else {
            // Check if a byte is available
            if sr.rxne().bit_is_set() {
                // Read the received byte
                Ok(usart.datar.read().dr().bits())
            } else {
                Err(nb::Error::WouldBlock)
            }
        }
    ...
    pub fn listen(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.rxneie().set_bit()) };
    }

    /// Stop listening for receive interrupt event
    pub fn unlisten(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.rxneie().clear_bit()) };
    }

    /// Start listening for idle interrupt event
    pub fn listen_idle(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.idleie().set_bit()) };
    }

    /// Stop listening for idle interrupt event
    pub fn unlisten_idle(&mut self) {
        unsafe { (*USART::ptr()).ctlr1.modify(|_, w| w.idleie().clear_bit()) };
    }

    /// Returns true if the line idle status is set
    pub fn is_idle(&self) -> bool {
        unsafe { (*USART::ptr()).statr.read().idle().bit_is_set() }
    }

    /// Returns true if the rx register is not empty (and can be read)
    pub fn is_rx_not_empty(&self) -> bool {
        unsafe { (*USART::ptr()).statr.read().rxne().bit_is_set() }
    }

    /// Clear idle line interrupt flag
    pub fn clear_idle_interrupt(&self) {
        unsafe {
            let _ = (*USART::ptr()).statr.read();
            let _ = (*USART::ptr()).datar.read();
        }
    }
}
        ...
        $(
            ...
            impl Rx<$USARTX> {
                pub fn with_dma(self, channel: $dmarxch) -> $rxdma {
                    unsafe { (*$USARTX::ptr()).ctlr3.modify(|_, w| w.dmar().set_bit()); }
                    RxDma {
                        payload: self,
                        channel,
                    }
            ...
            impl Tx<$USARTX> {
                pub fn with_dma(self, channel: $dmatxch) -> $txdma {
                    unsafe { (*$USARTX::ptr()).ctlr3.modify(|_, w| w.dmat().set_bit()); }
                    TxDma {
                        payload: self,
                        channel,
                    }
