...
/// Hardware timers
pub struct SysCounterHz(Timer<STK>);
...
impl SysCounterHz {
    /// Note: Although the qingke core support 64 bit systick time, we can't operate it atomically.
    /// So, this HAL crate only support lower 32 bits. It should be enough.
    pub fn start(&mut self, timeout: Hertz) -> Result<(), Error> {
        let rvr = self.clk.raw() / timeout.raw() - 1;


        self.tim.set_reload_low(rvr);
...
pub struct SysCounter<const FREQ: u32>(Timer<STK>);
...
impl<const FREQ: u32> SysCounter<FREQ> {
    ...
    pub fn now(&self) -> TimerInstantU32<FREQ> {
        TimerInstantU32::from_ticks(STK::get_current_low() / (self.clk.raw() / FREQ))
    }
